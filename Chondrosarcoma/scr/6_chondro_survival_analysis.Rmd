...
df_avg_immune_sign_z_scores <- read.table("../results/imm_sign_avg_z_scores.tsv", sep = "\t", header = TRUE)
rownames(df_avg_immune_sign_z_scores) <- df_avg_immune_sign_z_scores$Signature
df_avg_immune_sign_z_scores <- as.data.frame(t(df_avg_immune_sign_z_scores[, -1]))
df_avg_immune_sign_z_scores <- df_avg_immune_sign_z_scores[df_metadata_surv$Patient,]
df_avg_immune_sign_z_scores_conv <- df_avg_immune_sign_z_scores[df_metadata_surv_conv$Patient,]
...
medians_sign <- apply(df_avg_immune_sign_z_scores, 2, median)

# Add categorical data, LOW is for under expressed genes (under the median) and HIGH upreg
df_cat_all <- df_avg_immune_sign_z_scores
for (col in colnames(df_cat_all)) {
  new_column_name <- paste(col, "cat", sep = " ")
  df_cat_all[[new_column_name]] <- ifelse(df_avg_immune_sign_z_scores[[col]] > medians_sign[col], "HIGH", "LOW")
}
...
# Continuous data
df_cont_all <- df_avg_immune_sign_z_scores
...
# CTA data
df_expr <- read.table("../results/whole_gene_CTA_sign_imm_clean_avg_z_scores.tsv", sep = "\t", header = TRUE, check.names = FALSE)
rownames(df_expr) <- df_expr$SYMBOL
df_expr_cta <- subset(df_expr, CTA == "CTA")
...
#pdf("../results/figures/forest_plots/all_indiv/forest_plot_all_patients_var_cont_cta_zscores.pdf", width = 10, height = 30)
...
# Merge df_long avec les clusters
df_long <- pivot_longer(df, 
                        cols = -SYMBOL, 
                        values_to = "value")

# Ajouter l'information du cluster dans df_long
df_long <- merge(df_long, cluster_data, by = "SYMBOL")

# Maintenant on peut faire le graphique avec les couleurs par cluster
ggplot(df_long, aes(x = reorder(SYMBOL, -value), y = value, color = as.factor(Cluster))) +
  geom_jitter(width = 0.2, height = 0, size = 1, alpha = 0.3) +  # Ajouter la couleur par cluster
  geom_boxplot(alpha = 0.4, outliers = FALSE, colour = "black", fill = NA) +  # Boîtes sans remplissage mais avec couleur de contour
  labs(title = "Plot of intensities per significant coxph CTA", x = "CTA", y = "Intensities") +
  scale_color_manual(values = c("1" = "#EA4343", "2" = "blue", "3" = "#75E05A")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))  
...
```{r}
df_int <- df_CTA_immune_whole_clean_avg
