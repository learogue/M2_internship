...
df_indiv_clusters_hm <- data.frame(
  Cluster = c(rep(1, length(indiv_clust[1:18])), 
              rep(2, length(indiv_clust[19:52])),
...
```{r, fig.width=7, fig.height=7, fig.cap="Volcano plot of DEG between C1 and C2 (from fig. 6)"}
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C1 and C4 (from fig. 8)"}
# Select the clusters
# Extract clusters
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot ofdifferentially expressed CTA between C1 and C4 (from fig. 8)"}
...
#### i) SDK2
```{r, fig.cap="Intensities boxplot for SDK2 in C1 and C4 (from fig. 8)"}
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C1 and C4 (from fig. 14)"}
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of differentially expressed CTA between C1 and C4 (from fig. 14)"}
EnhancedVolcano(res_cta,
                lab = rownames(res_cta),
                pCutoff = 0.05,
                FCcutoff = 0.8,
...
### DEG analysis
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C1 and C2 (from fig. 17)"}
# Extract clusters from the previous heatmap
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of differentially expressed CTA between C1 and C2 (from fig. 17)"}
EnhancedVolcano(res_cta,
                lab = rownames(res_cta),
                pCutoff = 0.05,
                FCcutoff = 0.8,
...
### DEG analysis
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C1 and C2 (from fig. 21) "}
# Store clusters
# Take col indexes
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C2 and C3 (from fig. 21) "}
...
```{r, fig.width=8, fig.height=8, fig.cap="Volcano plot of DEG between C1 and C3 (from fig. 21) "}
...
### a- k = 2
```{r, fig.height=8, fig.width=15, fig.cap="Heatmap (from fig. 17) with metadata (n = 82)"}
# Heatmap
...
### b- k = 3
```{r, fig.height=8, fig.width=15, fig.cap="Heatmap (from fig. 21) with metadata (n = 82)"}
# Heatmap
#pdf("../results/figures/heatmaps/heatmap_complete_annotated_conv_patients.pdf", height = 10, width = 15)
set.seed(1)
heatmap_anno_conv <- Heatmap(
...
```{r, fig.height=8, fig.width=10, fig.cap="Dendrogram of conventionnal patients (from fig. 3)"}
...
# Re-cluster samples
sample_hclust <- hclust(dist(df), method = "average")
...
```{r, fig.width=10, fig.cap="Corrected dendrogram of conventionnal patients (from fig. 3)"}
# Delete Ly016
...
## 1) Threshold power selection for WGCNA
...
## 2) Gene clustering and module detection with the topological similarity matrix (TOM)
...
## 3) Relationship between gene modules and immune cell types: Correlation heatmap
